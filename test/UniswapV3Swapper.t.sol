...
contract UniswapV3SwapperTest is Test {
    ...
    function _registerPools() internal {
        ...
        priceProvider.registerPool(address(WETH), address(USDC), WETH_USDC_500_POOL, 500);

        // Register WETH/USDT pool (0.3% fee)
        priceProvider.registerPool(address(WETH), address(USDT), WETH_USDT_3000_POOL, 3000);

        // Register WETH/DAI pool (0.3% fee)
        priceProvider.registerPool(address(WETH), address(DAI), WETH_DAI_3000_POOL, 3000);

        // Register USDC/USDT pool (0.05% fee)
        priceProvider.registerPool(address(USDC), address(USDT), USDC_USDT_500_POOL, 500);

        // Register USDC/DAI pool (0.05% fee)
        priceProvider.registerPool(address(USDC), address(DAI), USDC_DAI_500_POOL, 500);
